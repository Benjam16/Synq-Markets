...
export default function PumpDetailPanel({
  mint,
  onClose,
}: {
  mint: string;
  ...
  const [lastQuoteResponse, setLastQuoteResponse] = useState<Record<string, unknown> | null>(null);
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        const res = await fetch(`/api/pump/quote?${params.toString()}`, { cache: 'no-store' });
        const json = await res.json();
        if (!res.ok || cancelled) {
        ...
        const qr = json?.quoteResponse;
        if (!qr || typeof qr !== 'object') {
          setExpectedReceive('');
          setLastQuoteResponse(null);
          setQuoteError('Invalid quote');
          return;
        }

        setLastQuoteResponse(qr as Record<string, unknown>);
        const rawOut = (qr as { outAmount?: string | number }).outAmount;
        const outAmountNum =
          typeof rawOut === 'string' ? Number(rawOut) : typeof rawOut === 'number' ? rawOut : NaN;
        const outDecimals =
        ...
        setExpectedReceive(ui.toLocaleString(undefined, { maximumFractionDigits: 6 }));
      } catch {
        if (!cancelled) {
          setExpectedReceive('');
          setLastQuoteResponse(null);
  ...
  const doSwap = async () => {
    ...
    try {
      ...
      const quoteRes = await fetch(`/api/pump/quote?${params.toString()}`, { cache: 'no-store' });
      const quoteJson = await quoteRes.json();
      if (!quoteRes.ok) {
        throw new Error(quoteJson?.error || 'Quote failed');
      }
      const quoteResponse = quoteJson?.quoteResponse;
      if (!quoteResponse || typeof quoteResponse !== 'object') {
        throw new Error('Quote missing');
      }

      const swapRes = await fetch('/api/pump/swap', {
        method: 'POST',
      ...
      try {
        if (typeof window !== 'undefined') {
          window.dispatchEvent(
            new CustomEvent('pump-trade-executed', {
              detail: {
                mint,
                side,
                quoteAsset,
      ...
      >
        ...
        <div className="p-6 space-y-4">
          <p className="text-[11px] text-slate-500 leading-relaxed">
            Trades route through <span className="text-slate-400">Jupiter</span> (Pump.fun pools on Solana).
            Ensure <span className="font-mono">JUPITER_API_KEY</span> is set for reliable quotes.
          </p>
          ...
          <button
            type="button"
            disabled={!connected || !publicKey || busy || !lastQuoteResponse}
